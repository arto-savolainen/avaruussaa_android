...
import com.example.avaruussaa_android.data.workers.PeriodicUpdateWorker;
import com.example.avaruussaa_android.data.workers.UpdateWorker;
import com.example.avaruussaa_android.interfaces.TimerSubscriber;

import java.util.Calendar;
import java.util.concurrent.TimeUnit;

// This class offers static functions for running the Workers which fetch data from web and update app state.
// It also runs a timer to the next update and accepts a subscriber that is notified on each tick of the timer.
public class WorkController {
    ...
    // Creates and runs a single UpdateWorker.
    ...
    // Enqueues a periodic work request which replaces any previous such request. This request runs PeriodicUpdateWorker
    ...
    // one minute after the source has updated (since it occasionally updates with a delay), so at 12.01, 12.11, 12.21, and so on.
